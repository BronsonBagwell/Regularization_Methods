...
```{r}
comp_ts <- rbind(LASSO = model_lasoperformance_ts,
                 RIDGE = model_rdgperformance_ts,
                 Net   = model_netperformance_ts)
print(comp_ts, digits = 4)
```
Ridge is the best with the lowest RMSE of 5.353 and highest RSquared of 0.7324. The Elastic Net search also selected alpha = 0 (pure Ridge), so it matches Ridge's performance, with the shared best tune shown below.
